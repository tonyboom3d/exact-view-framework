...
const ThankYou = ({ orderNumber, referralCode, selections, guests, buyer, showPayer }: ThankYouProps) => {
  ...
  return (
    <div className="space-y-5 text-center">
      ...
      <div>
        <h2 className="text-[25px] font-bold text-foreground">ההזמנה התקבלה!</h2>
        <p className="text-[15px] text-muted-foreground mt-1">מספר הזמנה: {orderNumber}</p>
      </div>
      ...
      <div className="rounded-xl border border-border bg-background p-4 text-right space-y-3">
        <p className="text-[15px] font-bold text-foreground">סיכום הזמנה</p>
        ...
        <div className="space-y-1.5">
          {activeSelections.map((s) => {
            ...
            return (
               <div key={s.type} className="flex items-center justify-between text-[15px]">
        ...
        <div className="border-t border-border pt-2 flex items-center justify-between">
           <span className="text-[15px] font-bold text-foreground">סה״כ ({totalTickets} כרטיסים)</span>
        ...
        <button
          ...
          className="flex items-center gap-1.5 text-[12px] text-primary font-medium cursor-pointer mx-auto"
        >
              ...
              <div className="space-y-3 pt-2 border-t border-border">
                {guests.slice(0, totalTickets).map((guest, idx) => {
                  ...
                  return (
                    <div key={idx} className="bg-muted/30 rounded-lg p-3 space-y-1">
                       <p className="text-[12px] font-bold text-foreground">{ticketLabel}</p>
                       <p className="text-[12px] text-muted-foreground">{guest.firstName} {guest.lastName}</p>
                       <p className="text-[12px] text-muted-foreground" dir="ltr" style={{ textAlign: 'right' }}>{guest.phone}</p>
                       {guest.email && <p className="text-[12px] text-muted-foreground" dir="ltr" style={{ textAlign: 'right' }}>{guest.email}</p>}
                    </div>
                  );
                })}
                ...
                {showPayer && (buyer.firstName || buyer.lastName) && (
                  <div className="bg-primary/5 rounded-lg p-3 space-y-1 border border-primary/20">
                     <p className="text-[12px] font-bold text-foreground">פרטי המשלם</p>
                     <p className="text-[12px] text-muted-foreground">{buyer.firstName} {buyer.lastName}</p>
                     <p className="text-[12px] text-muted-foreground" dir="ltr" style={{ textAlign: 'right' }}>{buyer.phone}</p>
                     {buyer.email && <p className="text-[12px] text-muted-foreground" dir="ltr" style={{ textAlign: 'right' }}>{buyer.email}</p>}
                  </div>
                )}
              </div>
            </motion.div>
      ...
      <div className="rounded-xl border-2 border-dashed border-[hsl(var(--cta))]/30 bg-[hsl(var(--cta))]/5 p-5 space-y-3">
        <div className="flex items-center justify-center gap-2">
          ...
          <p className="text-[19px] font-extrabold text-foreground">Win - Win</p>
        </div>
        ...
        <div className="bg-background rounded-lg p-3 flex items-center gap-2" dir="ltr">
          <input
            readOnly
            value={referralLink}
            className="flex-1 text-[12px] bg-transparent outline-none text-muted-foreground"
          />
        ...
        <div className="grid grid-cols-2 gap-2">
          <Button
            onClick={copyLink}
            variant="outline"
             className="gap-1.5 text-[12px] h-10 font-medium"
          >
          ...
          <Button
            ...
            className="gap-1.5 text-[12px] h-10 font-medium bg-[hsl(142,70%,45%)] hover:bg-[hsl(142,70%,40%)] text-white"
          >
          ...
          <Button
            onClick={shareEmail}
            variant="outline"
            className="gap-1.5 text-[12px] h-10 font-medium col-span-2"
          >
